...
Feature: Processing iNaturalist observations sanitises invalid HTML links
	...
	Rule: Invalid html references are removed with a warning
		...
		Then the visits record has 'Notes' set to 'Map Link (INVALID URL DETECTED - see iNaturalist link for full notes)'
		...
		When we process the observation
		Then the visits record has 'Notes' set to 'Link1: (INVALID URL DETECTED - see iNaturalist link for full notes) words here. Link2: (INVALID URL DETECTED - see iNaturalist link for full notes)'
		...
		When we process the observation
		Then the visits record has 'Notes' set to '<div>words=morewords</div>'
		...
		When we process the observation
		Then the visits record has 'Notes' set to '<a href="http://www.madeup.com">words words</a>'